...
def pay(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj.save()
            sign = hashlib.md5("SkyZmey:%s:%d:Pxh3dzVg5bayPQvXwz2v" % (str(obj.amount), obj.id)).hexdigest()
...
def result(request):
    ...
    if sign.lower() != hashlib.md5("SkyZmey:%s:%s:2YwKcqQgRCxLALPJBE35" % (amount, id)).hexdigest().lower():
        raise Http404()
