...
class ChainedSelect(Select):
    ...
    def render(self, name, value, attrs=None, choices=(), prefix = None):
        if prefix:
            url = "/".join(reverse("simplechained_filter", kwargs={'app':self.app_name,'model':self.model_name,'field':self.model_field,'value':"1"}).split("/")[:-2])
        else:
...
class SimpleChainedSelect(ChainedSelect):
    def render(self, name, value, attrs=None, choices=(), prefix = None):
        js = """<script type="text/javascript">"""
        
        i = 0
        for item in self.chain_field:
            url = "/".join(reverse("simplechained_filter", kwargs={'app':self.app_name,'model':self.model_name,'field':self.model_field[i],'value':"1"}).split("/")[:-2])
            js += '''
            $(function(){
                $("select#id_%(model)s").change(function(){
                $.getJSON("%(url)s/"+$(this).val()+"/", function(j){
                    var options = '';
                    for (var i = 0; i < j.length; i++) {
                        options += '<option value="' + j[i].value + '">' + j[i].display + '</option>';
                    }
                    $("#id_%(chainfield)s").html(options);
                    $('#id_%(chainfield)s option:first').attr('selected', 'selected');
                })
            })
            })
            ''' % {"chainfield":item, "url":url, "id":attrs['id'], 'model': self.model_name.lower()}
            i += 1
...
#            self.choices = [choice]
#            break
        output = super(ChainedSelect, self).render(name, value, attrs)
        output += js
        return mark_safe(output)
